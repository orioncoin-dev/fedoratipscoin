...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([qt-incdir],[AS_HELP_STRING([--with-qt-incdir=INC_DIR],[specify qt include path])], [qt_include_path=$withval], [])
  AC_ARG_WITH([qt-libdir],[AS_HELP_STRING([--with-qt-libdir=LIB_DIR],[specify qt lib path])], [qt_lib_path=$withval], [])
  AC_ARG_WITH([qt-plugindir],[AS_HELP_STRING([--with-qt-plugindir=PLUGIN_DIR],[specify qt plugin path])], [qt_plugin_path=$withval], [])
...
dnl  if test x$use_pkgconfig = xyes; then
dnl    if test x$PKG_CONFIG == x; then
dnl      AC_MSG_ERROR(pkg-config not found.)
dnl    fi
dnl    BITCOIN_QT_CHECK([_BITCOIN_QT_FIND_LIBS_WITH_PKGCONFIG([$2])])
dnl  else
dnl    BITCOIN_QT_CHECK([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG])
dnl  fi
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
  if test x$bitcoin_qt_got_major_vers = x5; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       PKG_CHECK_MODULES([QTPLATFORM], [Qt5AccessibilitySupport], [QT_LIBS="$QTPLATFORM_LIBS $QT_LIBS"])
       ...
       if test x$TARGET_OS = xwindows; then
         ...
         if test x$bitcoin_cv_need_platformsupport = xyes; then
           BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}AccessibilitySupport],[main],,BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXPlatformSupport not found)))
         fi
       fi
     fi
  else
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([AC_CHECK_HEADER([QtCore/QtPlugin],,BITCOIN_QT_FAIL(QtCore headers missing))])
  BITCOIN_QT_CHECK([AC_CHECK_HEADER([QtWidgets/QApplication],, BITCOIN_QT_FAIL(QtGui headers missing))])
  BITCOIN_QT_CHECK([AC_CHECK_HEADER([QtNetwork/QLocalSocket],, BITCOIN_QT_FAIL(QtNetwork headers missing))])
    ...
    if test x$bitcoin_qt_got_major_vers == x5; then
      ...
      if test x$bitcoin_cv_static_qt == xyes; then 
        ...
        if test x$TARGET_OS == xwindows; then
          ...
          dnl _BITCOIN_QT_CHECK_STATIC_PLUGINS([Q_IMPORT_PLUGIN(QWindowsPrinterSupportPlugin)],[-lwindowsprintersupport])
        fi
      fi
    else
      AC_DEFINE(QT_STATICPLUGIN, 1, [Define this symbol if qt plugins are static])
